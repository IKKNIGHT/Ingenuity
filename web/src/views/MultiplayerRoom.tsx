...
export function MultiplayerRoom({ roomCode, displayName, onBack }: Props) {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 pb-20 pt-8">
      ...
      {!snap && !connError && (
        <p className="text-center text-slate-400">Connecting to the lobby…</p>
      )}
      {!snap && connError && (
        <div className="mx-auto max-w-lg rounded-2xl border border-rose-500/40 bg-rose-950/40 p-6 text-center text-sm text-rose-100">
          <p>{connError}</p>
          <button
            type="button"
            onClick={onBack}
      ...
      {snap && snap.phase === "playing" && (
        ...
        >
          {snap.you.participates && (
            <section className="rounded-3xl border border-white/10 bg-[var(--color-panel)] p-5 shadow-xl backdrop-blur-md">
          ...
          {!snap.you.participates && (
            <p className="mb-4 rounded-2xl bg-amber-950/40 px-4 py-3 text-sm text-amber-100 ring-1 ring-amber-500/30">
          ...
          <section
            className={`rounded-3xl border border-white/10 bg-black/20 p-5 ring-1 ring-white/5 ${
              snap.you.participates ? "" : "lg:col-span-2"
            }`}
          >
            <h2 className="mb-4 text-sm font-semibold uppercase tracking-wider text-slate-500">
              {snap.you.isHost ? "All players (read-only)" : snap.you.participates ? "Other players" : "Live boards"}
            </h2>
            <div className="grid max-h-[70vh] gap-6 overflow-y-auto pr-1 sm:grid-cols-2 lg:grid-cols-3">
              {watchBoards.map(([id, p]) => (
                <div key={id} className="rounded-2xl bg-white/5 p-3 ring-1 ring-white/10">
                  ...
                  <PuzzleBoard state={p.state!} readOnly compact />
...
function LobbyPanel({
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-2">
      <div className="rounded-3xl border border-white/10 bg-[var(--color-panel)] p-6 backdrop-blur-md">
      ...
      {snap.you.isHost ? (
        <div className="rounded-3xl border border-teal-500/20 bg-teal-950/30 p-6 backdrop-blur-md">
        ...
        <div className="rounded-3xl border border-white/10 bg-[var(--color-panel)] p-6 text-sm text-slate-400 backdrop-blur-md">
...
function Results({
  ...
  if (standings.length === 0) {
    return (
      <div className="rounded-3xl border border-white/10 bg-[var(--color-panel)] p-8 text-center text-slate-400 backdrop-blur-md">
